...
class Answer_filtering_module(torch.nn.Module):
    ...
    def forward(self, question, questions_length, head_entity, tail_entity, max_sent_len, loader):
        ...
        top_k_loss_factor = 0.0005

        ranked_topK_entity_idxs = self.get_ranked_top_k(question, questions_length, head_entity, max_sent_len, 5)
        ranked_topK_entity_idxs = ranked_topK_entity_idxs.indices.tolist()
        top_k_loss = self.correct_entity_type(ranked_topK_entity_idxs, answers_entity_type, pred_answers_score)

        loss = bce_loss + triple_loss_factor*contrastive_loss + top_k_loss_factor*top_k_loss
    ...
    def correct_entity_type(self, topK_entity_idxs, answer_type, pred_answers_score):
        loss = 0.0
        for idx in range(len(topK_entity_idxs)):
            top_k_idxs = topK_entity_idxs[idx]
            for k in top_k_idxs:
                entity_type = kg.ENTITY_2_ENTITY_TYPE[k]
                if entity_type != answer_type[idx]:
                    loss += pred_answers_score[idx][k]
        return loss    